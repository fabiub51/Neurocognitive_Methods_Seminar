function normalise_pre(data_dir,source_dir)

normalise = struct; 
files_n = spm_select('List',data_dir,'^fM','.img');
fs_n = cellstr([repmat([data_dir filesep], size(files_n,1), 1) files_n, repmat(',1',size(files_n,1),1)]);
